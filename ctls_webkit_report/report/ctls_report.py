...
class ctls_report(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(ctls_report, self).__init__(cr, uid, name, context)
        self.count = 0
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        return super(ctls_report, self).set_context(objects, data, ids, report_type=report_type)
            ...
            if payslip_ids:
                for payslip in payslip_line.browse(self.cr, self.uid, payslip_ids):
                    for line in payslip.line_ids:
                        if line.name == 'CTLS':
                            self.amount_ctls = line.amount
            self.counter = self.counter + 1                
            result.append({
            'name' :  i['employee'],   
            'ctls' : self.amount_ctls,
            'rank_id': i['rank_id'],
            'grade_id': i['grade_id'],
...
report_sxw.report_sxw('report.ctls.date.webkit.report', 'hr.payslip', 'addons/ctls_webkit_report/report/ctls_report.mako', parser=ctls_report, header=False)
