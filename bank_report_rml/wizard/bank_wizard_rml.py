...
class bank_wizard_rml(osv.osv_memory):
    _name ='bank.wizard.rml'

    _columns = {
         'payment_code': fields.char('Payment Code', required=True),
         'period_id': fields.many2one('account.period', 'Period',  required=True),
         'bank_ids': fields.many2many('res.bank', 'employee_bank_rel_d', 'bank_id', 'employee_id', 'Banks'),
    ...
    def print_report(self, cr, uid, ids, context=None):
        ...
        if data['acc_unit'] and data['account_type'] == False:
            return {
                ...
                'report_name': 'payroll.bank.all.rml.acc.unit',
                'datas': datas,
            }
        elif data['account_type'] and data['acc_unit'] == False:
            return {
                'type': 'ir.actions.report.xml',
                'report_name': 'payroll.bank.all.rml.account.type',
                'datas': datas,
            }
        elif data['acc_unit'] and data['account_type']:
            return {
                'type': 'ir.actions.report.xml',
                'report_name': 'payroll.bank.all.rml.rml',
                'datas': datas,
            }
        else:
            return {
                'type': 'ir.actions.report.xml',
                'report_name': 'payroll.bank.all.rml.without.type.unit',
                'datas': datas,
            }
