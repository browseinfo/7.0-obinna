...
class bank_report_rml(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(bank_report_rml, self).__init__(cr, uid, name, context)
        self.counter = 0
        self.emp_one_name = ''
        self.emp_two_name = ''
    ...
    def set_context(self, objects, data, ids, report_type=None):
        ...
        period_id = period_obj.search(self.cr,self.uid, [('name', '=', data['form']['period_id'][1] )])
        self.bank_idsss = data['form']['bank_ids']
        self.payment_code = data['form']['payment_code']
        for period in period_obj.browse(self.cr, self.uid, period_id):
            self.date_from = period.date_start
            self.date_to = period.date_stop
        return super(bank_report_rml, self).set_context(objects, data, ids, report_type=report_type)
    
        
    def _call_date_from(self):
        date = datetime.strptime(self.date_from, '%Y-%m-%d')
        return (str(date.day) + '/' + str(date.month) + '/' + str(date.year))
        
    def _call_date_to(self):
    ...
    def _get_bank_name(self):
        for bank_id in self.bank_idsss:
            self.cr.execute("select name from res_bank where id = %s", (bank_id,))
        name = str(self.cr.fetchone())
        if "(u'" in name:
            if " - " in name:
                name = name.split(" - ")
                bank_name = '( ' + name[0].split("u'")[1] + ' )'
                return bank_name
            else:
                name = name.split("(u")
                name = name[1].split("',)")
    ...
    def _get_payslip_lines(self, obj, column_flag=0):
        ...
        print "\n\n=========obj==========", obj
        self.total = 0.0
        self.bank = self.bank_idsss
        city = ''
        street = ''
        
        for bank in self.bank_idsss:
            self.cr.execute("select id from res_partner_bank where bank in (select id from res_bank where id = %s)", (bank,))
            ...
            for line in payslip_line.browse(self.cr, self.uid, line_name):
                ...
                if grade_name and step_name:
                    ...
                    print "\n\n============wages['net_emoluments']=================", wages, wages['net_emoluments']
                    res.append({
...
for suffix in ['.account.type', '', '.acc.unit', '.without.type.unit']:
    if suffix == '.account.type':
        report_sxw.report_sxw('report.payroll.bank.all.rml' + suffix,
                              'bank.wizard.rml', 
                              'addons/bank_report_rml/report/bank_report_account_type.rml', 
                              parser=bank_report_rml, 
                              header=False)
    elif suffix == '':
        report_sxw.report_sxw('report.payroll.bank.all.rml.rml' + suffix,
                              'bank.wizard.rml', 
                              'addons/bank_report_rml/report/bank_report_rml.rml', 
                              parser=bank_report_rml, 
                              header=False)
        ...
        report_sxw.report_sxw('report.payroll.bank.all.rml' + suffix,
                              'bank.wizard.rml', 
                              'addons/bank_report_rml/report/bank_report_acc_unit.rml', 
                              parser=bank_report_rml, 
                              header=False)
    elif suffix == '.without.type.unit':
        report_sxw.report_sxw('report.payroll.bank.all.rml' + suffix,
                              'bank.wizard.rml', 
                              'addons/bank_report_rml/report/bank_report_without_rml.rml', 
                              parser=bank_report_rml, 
                              header=False)
